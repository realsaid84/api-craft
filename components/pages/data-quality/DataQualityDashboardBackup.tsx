...
export const DataQualityDashboard: React.FC<DataQualityDashboardProps> = ({ 
  ...
  const downloadReport = () => {
    const report = {
      ...
      apiName: (dataModel || spec)?.info?.title || "Untitled Data Model",
      metrics,
      violations,
      securityChecks,
    ...
    a.href = url;
    a.download = `api-quality-report-${(dataModel || spec)?.info?.title || "untitled"}.json`;
                  ...
                  <div className="flex flex-col md:flex-row items-center justify-between">
                    <div className="mb-4 md:mb-0">
                      <h2 className="text-xl font-semibold text-gray-700 mb-2">
                        {(dataModel || spec)?.info?.title || "Untitled Data Model"}
                      </h2>
                      <p className="text-gray-500 text-sm">
                        Version: {(dataModel || spec)?.info?.version || "N/A"} • 
                        Spec: {(dataModel || spec)?.openapi || (dataModel || spec)?.swagger || "Unknown"}
                      </p>
                    </div>
            ...
            <TabsContent value="overview">
              <div className="border-t border-gray-200 pt-4 mb-6">
                <div className="flex flex-wrap items-center gap-4 text-sm text-gray-600">
                  <div className="flex items-center gap-2">
                    <span>Size:</span>
                    <span className="text-blue-500 font-medium">{Math.round(JSON.stringify(dataModel || spec).length / 1024)}kb</span>
                  </div>
                  <div className="flex items-center gap-2">
                    <span>Version:</span>
                    <span className="text-blue-500 font-medium">{(dataModel || spec)?.openapi || (dataModel || spec)?.swagger || "Unknown"}</span>
                  </div>
                  <div className="flex items-center gap-2">
                    <span>Schemas:</span>
                    <span className="text-blue-500 font-medium">
                      {Object.keys((dataModel || spec)?.components?.schemas || {}).length}
                    </span>
                  </div>
                  <div className="flex items-center gap-2">
                    <span>Parameters:</span>
                    <span className="text-blue-500 font-medium">
                      {Object.keys((dataModel || spec)?.components?.parameters || {}).length}
                    </span>
                  </div>
                  <div className="flex items-center gap-2">
                    <span>Operations:</span>
                    <span className="text-blue-500 font-medium">
                      {Object.entries((dataModel || spec)?.paths || {}).reduce((count, [_, pathObj]) => {
                      ...
                      <div className="space-y-4">
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                          ...
                          <Card className="border-gray-200">
                            ...
                            <CardContent>
                              <div className="text-2xl font-bold">
                                {Object.keys((dataModel || spec)?.paths || {}).length}
                              </div>
                              <div className="mt-2 text-sm text-gray-500">
